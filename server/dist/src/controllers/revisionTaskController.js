...
export const createRevisionTask = async (req, res) => {
    try {
        ...
        const data = { title, duration_required, userId };
        if (priority)
            data.priority = priority;
        if (date)
            data.date = new Date(date);
        if (originalTaskId)
            data.originalTaskId = originalTaskId;
...
export const completeRevisionTask = async (req, res) => {
    try {
        const userId = getUserId(req);
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
...
export const deleteRevisionTask = async (req, res) => {
    try {
        const userId = getUserId(req);
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
...
//# sourceMappingURL=revisionTaskController.js.map